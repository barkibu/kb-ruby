...
- Every `KB::Client` now shares one Faraday connection per KB origin and API key (`KB::Connections`), so all model clients use one connection pool per process. URLs, headers and cache keys are unchanged. Settings are read when that connection is built, on the process's first KB call; `KB::Connections.reset!` applies later changes.
